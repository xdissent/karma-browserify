...
framework = (files, config={}) ->
  ...
  fileName = __dirname.replace(/[\\\/]/g, '_').replace('_node_modules_karma-browserify_lib', '');
  fileName = fileName + '_karma-browerify.js';
  tmp = path.join (if os.tmpdir then os.tmpdir() else os.tmpDir()), fileName
